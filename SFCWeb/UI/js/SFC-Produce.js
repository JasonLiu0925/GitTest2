...
var OKQty = 0;
var NGQty = 0;
var fitting=false; //配件
var exModel=false; //配件顯次
var gridList = [{
    //id:0
    //grid初始化參數
    caption: '',
...
{ //grid初始化參數
    ...
    //pager: '#jqGridFNPager',
    shrinkToFit: true,
    colAPI: 'SFIS_GET_FN_DATA_LIST', //set colModel index
    fixedColFDb: true, //set db
    //gridDefinitionUrl: invokeURL + 'SFIS_DIP01_SKU_Finished_LIST',
    gridDefinitionUrl: invokeURL + 'SFIS_GET_FN_DATA_LIST',
    //gridDefColionUrl: 'json/unSchedule.json',
    gridDefPostData: {
...
{ //grid初始化參數
   ...
   // pager: '#jqGridEXPager',
    shrinkToFit: true,
    colAPI: 'SFIS_GET_EX_DATA_LIST', //set colModel index
    fixedColFDb: true, //set db
    //gridDefinitionUrl: invokeURL + 'SFIS_DIP01_SKU_Finished_LIST',
    gridDefinitionUrl: invokeURL + 'SFIS_GET_EX_DATA_LIST',
    //gridDefColionUrl: 'json/unSchedule.json',
    gridDefPostData: {
...
function frmEvent() { //form event function
    ...
    $('#FNButton').on('click', function () {
        $('#FNModal').modal('show');
        createGrid(7);

    });
    $('#exTxt').on('keyup', function (event) {
        ...
        if (event.keyCode == 13) {
            ...
            if ( $('#exTxt').val() == '') {
                ...
                if(isPart){
                    ...
                    for (let i = 0; i < ids.length; i++) {
                        let row = $grid.jqGrid('getRowData', ids[i]);
                        if (row['VF003'] == ex  && row['VF003X']!=ex ) {
                            $grid.jqGrid('setCell', ids[i], 'VF003X', ex);
                            setPart = true;
                        }
                    ...
                    if(setPart){  //檢查是否滿了
                        ...
                        $('#exerrMsg').text( '此配件:' + ex + ' 已有輸入,不可重複輸入!!');
                    }
                }
                else{
                    $('#exerrMsg').text( '此配件:' + ex + '不屬於此工單,不可輸入!!');
                }
...
function woDispaly() {
    ...
    if (wocookie != "") {
        ...
        result = ajaxGetData(invokeURL + "SFIS_CHECK_FN_DATA_LIST", { //FN ENABLE

        });
        if (result[0].TOTAL > 0) {

            result = ajaxGetData(invokeURL + "SFIS_GET_FN_DATA_LIST", { //回傳FN資料
                MO: woList.wo
            });
            if (result[0].result == undefined) {
                fnCHK = true;
        ...
        if (fnCHK) {
            $('#FNButton').removeAttr('disabled');

        } else {
            $('#FNButton').attr('disabled', 'disabled');
...
function editLink(cellValue, options, rowObject, action) {
    //return "<a href='javascript:void(0)' style='color:blue;'onclick=gotoPost('"+rowObject.id+"')>"+cellValue+"</a>";
    return "<a href='" + cellValue + "' style='color:blue;' target='_blank'>" + cellValue + "</a>";
}
function gridColdisPlay(options) {
    switch (options) {
        case 0: //顯示列印
            $('#okBox').removeClass("col-md-6");
            $('#NGBox').removeClass("col-md-6");
            $('#rprintBox').removeClass("col");
            $('#okBox').addClass("col-md-5");
            $('#NGBox').addClass("col-md-5");
            $('#rprintBox').addClass("col-md-2");
            break;
        case 1: //不顯示列印
            $('#okBox').removeClass("col-md-5");
            $('#NGBox').removeClass("col-md-5");
            $('#rprintBox').removeClass("col-md-2");
            $('#okBox').addClass("col-md-6");
            $('#NGBox').addClass("col-md-6");
            $('#rprintBox').addClass("col");
            break;
    }
}
  function showEx(){
    let status
    let result = ajaxGetData(invokeURL + "SFIS_GET_EX_DATA_LIST", { //過站
        WO: $('#woDispy').val(),
        //SKU:$('#skuDisplay').val(),
        SKU:'1104A0242300',
       // STATION:$('#processCodeDispy').val()

    ...
    $('#searchBtn1').on('click', function () {
		...
		if($('#TYPE_1').val()=='2'){//鋼板
            ...
            if($('#stocktype').val()=='2'){//出庫
                    ...
                    // 定義包含工單的字符串
                    var workOrderString = $('#WO').val().trim();//"A工單,B工單,C工單,D工單"
                            
                    // 使用split方法拆分字符串，以逗號為分隔符
                    var workOrdersArray = workOrderString.split(",");
                    ...
                    for (var i = 0; i < workOrdersArray.length; i++) {
                        // 在網頁上顯示每個工單
                        // $("#workOrders").append("<p>" + workOrdersArray[i] + "</p>");


                        if( workOrdersArray[i]==null){
                            break;
                        }
                        ...
                        if (result[0]["RESULT"] != "OK") {
                            ...
                            resultMsg +=result[0]["MSG"]+','
                            alert(workOrdersArray[i]+" 出庫失敗:"+result[0]["MSG"]);
                            return;
                        }else{

                            resultMsg +=result[0]["MSG"]+','

                            // alert("出庫成功:"+result[0]["MSG"]); 
                        }
                    ...
                    if (resultMsg.endsWith(",")) {
                        resultMsg = resultMsg.slice(0, -1);
                    }

                    var strArray = resultMsg.split(","); // 使用逗號分割字符串成為數組
                    var uniqueStrArray = [...new Set(strArray)]; // 使用Set來去除重複的值
                    result_wo_data = uniqueStrArray.join(","); // 將數組轉換回字符串


                    alert("出庫成功!出庫的製令:"+result_wo_data); 
